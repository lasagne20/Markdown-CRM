# Documentation Index

## 📚 Complete Documentation

Welcome to the Markdown CRM documentation! This index provides an overview of all available resources.

---

## 🚀 Getting Started

### For New Users
1. [Why Markdown CRM?](./Why-Markdown-CRM.md) - Understand the philosophy and benefits
2. [Installation Guide](./Installation.md) - Get up and running quickly
3. [Quick Start](../README.md) - Your first steps with the system

### For Developers
1. [Architecture Overview](./Architecture.md) - Understand the system design
2. [Property Types Reference](./Property-Types.md) - All available property types
3. [Display Configuration](./Display-Configuration.md) - Configure class displays

---

## 📖 Core Documentation

### User Guides
- **[Why Markdown CRM?](./Why-Markdown-CRM.md)** - Philosophy, benefits, and use cases
- **[Installation Guide](./Installation.md)** - Step-by-step installation
- **[Display Configuration](./Display-Configuration.md)** - Configure how classes are displayed

### Technical Documentation
- **[System Architecture](./Architecture.md)** - System design and components
- **[Property Types](./Property-Types.md)** - Complete property type reference
- **[Data Loading](./Data-Loading.md)** - How data is loaded and managed
- **[YAML Configuration Format](./YAML-Configuration-Format.md)** - Property naming and configuration guide

### Property Guides
- **[Static Properties](./Static-Properties.md)** - Non-editable fields guide
- **[Property Aliases](./Property-Aliases.md)** - Alternative property names
- **[Classe Property Aliases](./ClassePropertyAliases.md)** - Migration of legacy files with different classe property names

### Feature Guides
- **[Populate Feature](./Populate-Feature.md)** - Interactive file creation with property prompts
- **[Auto-Rename](./Auto-Rename.md)** - Automatic file renaming based on properties
- **[Process System](./Process-System.md)** - Automated actions based on triggers and conditions
- **[Quick Reference: Static Properties](./QUICK-REFERENCE-Static-Properties.md)** - Quick snippets and examples

---

## 🆕 Latest Updates

### November 2025 Release
- **[Release Notes](../RELEASE-NOTES.md)** - What's new in this release
- **[Migration Guide](../MIGRATION-GUIDE.md)** - How to upgrade
- **[Test Fixes & Improvements](./Test-Fixes-Nov-2025.md)** - Detailed technical changelog
- **[Changelog](../CHANGELOG.md)** - Complete version history

---

## 📑 Documentation by Topic

### Configuration
- [Display Configuration](./Display-Configuration.md) - Visual display setup
- [YAML Configuration Format](./YAML-Configuration-Format.md) - Property naming and configuration
- [Static Properties](./Static-Properties.md) - Non-editable fields
- [Populate Feature](./Populate-Feature.md) - Interactive file creation
- [Property Types](./Property-Types.md) - Available property types
- [Process System](./Process-System.md) - Automated actions with triggers and conditions

### Development
- [Architecture](./Architecture.md) - System design
- [Data Loading](./Data-Loading.md) - Data management
- [Test Fixes](./Test-Fixes-Nov-2025.md) - Test suite documentation

### Maintenance
- [Migration Guide](../MIGRATION-GUIDE.md) - Upgrade instructions
- [Changelog](../CHANGELOG.md) - Version history
- [Release Notes](../RELEASE-NOTES.md) - Release summaries

---

## 🎯 Documentation by Role

### I'm a Product Owner / Decision Maker
**Start here:**
1. [Why Markdown CRM?](./Why-Markdown-CRM.md) - Understand the value proposition
2. [Release Notes](../RELEASE-NOTES.md) - See what's new
3. [Installation Guide](./Installation.md) - Try it out

**Key metrics:**
- ✅ 1081/1081 tests passing (100%)
- 📊 87% code coverage
- ⚡ Fast execution (~2.8s for full test suite)

### I'm a User / Content Manager
**Start here:**
1. [Installation Guide](./Installation.md) - Get set up
2. [Display Configuration](./Display-Configuration.md) - Configure your classes
3. [Populate Feature](./Populate-Feature.md) - Create files interactively
4. [Quick Reference: Static Properties](./QUICK-REFERENCE-Static-Properties.md) - Use non-editable fields

**Key features:**
- 📝 Markdown-based data storage
- 🎨 Customizable displays
- 🚀 Interactive file creation with populate
- 🔒 Protected fields (static properties)

### I'm a Developer / Integrator
**Start here:**
1. [Architecture](./Architecture.md) - Understand the system
2. [Property Types](./Property-Types.md) - Available property types
3. [Data Loading](./Data-Loading.md) - Data management

**Key resources:**
- 🧪 [Test Fixes](./Test-Fixes-Nov-2025.md) - Complete test documentation
- 📦 [Migration Guide](../MIGRATION-GUIDE.md) - Upgrade procedures
- 🔧 [Quick Reference](./QUICK-REFERENCE-Static-Properties.md) - Code snippets

---

## 📊 Quick Stats

### Documentation Coverage
- **User Guides**: 3 documents
- **Technical Docs**: 5 documents
- **Release Docs**: 4 documents
- **Total**: 12+ comprehensive documents

### Code Quality
- **Tests**: 1081/1081 passing ✅
- **Test Suites**: 45/45 passing ✅
- **Coverage**: ~87%
- **Execution**: ~2.8s

### Latest Release (November 2025)
- **New Features**: Static properties
- **Tests Added**: 20 new tests
- **Tests Fixed**: 44 tests adapted
- **Files Modified**: 16 files

---

## 🔍 Find What You Need

### By Document Type

#### Guides & Tutorials
- [Installation Guide](./Installation.md)
- [Display Configuration](./Display-Configuration.md)
- [YAML Configuration Format](./YAML-Configuration-Format.md)
- [Static Properties Guide](./Static-Properties.md)
- [Populate Feature Guide](./Populate-Feature.md)
- [Migration Guide](../MIGRATION-GUIDE.md)

#### Reference Documentation
- [Property Types Reference](./Property-Types.md)
- [Quick Reference: Static Properties](./QUICK-REFERENCE-Static-Properties.md)
- [Architecture Reference](./Architecture.md)

#### Release Information
- [Release Notes](../RELEASE-NOTES.md)
- [Changelog](../CHANGELOG.md)
- [Test Fixes](./Test-Fixes-Nov-2025.md)

#### Conceptual Documentation
- [Why Markdown CRM?](./Why-Markdown-CRM.md)
- [Data Loading](./Data-Loading.md)
- [Architecture](./Architecture.md)

---

## 🛠️ Quick Links

### Common Tasks
- **Installing**: [Installation Guide](./Installation.md)
- **Configuring**: [Display Configuration](./Display-Configuration.md)
- **Upgrading**: [Migration Guide](../MIGRATION-GUIDE.md)
- **Understanding**: [Architecture](./Architecture.md)

### Common Questions
- **What is Markdown CRM?**: [Why Markdown CRM?](./Why-Markdown-CRM.md)
- **What's new?**: [Release Notes](../RELEASE-NOTES.md)
- **How to create files interactively?**: [Populate Feature](./Populate-Feature.md)
- **How to make fields read-only?**: [Static Properties](./Static-Properties.md)
- **What properties are available?**: [Property Types](./Property-Types.md)

---

## 📝 Contributing to Documentation

### Documentation Guidelines
1. Write in clear, simple English
2. Include code examples where relevant
3. Use consistent formatting (Markdown)
4. Keep documents focused and concise
5. Link to related documents

### File Organization
```
docs/
├── INDEX.md                              # This file
├── README.md                             # Documentation overview
├── Why-Markdown-CRM.md                   # Philosophy
├── Installation.md                       # Installation guide
├── Architecture.md                       # System architecture
├── Data-Loading.md                       # Data management
├── Display-Configuration.md              # Display setup
├── YAML-Configuration-Format.md          # Property naming system
├── Property-Types.md                     # Property reference
├── Static-Properties.md                  # Static properties guide
├── Populate-Feature.md                   # Populate feature guide
├── QUICK-REFERENCE-Static-Properties.md  # Quick reference
└── Test-Fixes-Nov-2025.md               # Test documentation

Root:
├── README.md                             # Project overview
├── CHANGELOG.md                          # Version history
├── RELEASE-NOTES.md                      # Release summary
└── MIGRATION-GUIDE.md                    # Upgrade guide
```

---

## 🔗 External Resources

### Project Links
- **GitHub Repository**: [Your repo URL here]
- **Issue Tracker**: [Issues URL here]
- **Discussions**: [Discussions URL here]

### Community
- **Obsidian Forum**: [Obsidian community]
- **Markdown Resources**: [Markdown guide]

---

## 📞 Getting Help

### Need Help?
1. Check this documentation index
2. Search the [Issue Tracker]
3. Ask in [Discussions]
4. Read the [FAQ]

### Found a Bug?
1. Check [known issues]
2. Review [Test Fixes](./Test-Fixes-Nov-2025.md)
3. Submit an issue with:
   - Steps to reproduce
   - Expected behavior
   - Actual behavior
   - System information

---

## 🎓 Learning Paths

### Beginner Path
1. [Why Markdown CRM?](./Why-Markdown-CRM.md) - 5 min read
2. [Installation Guide](./Installation.md) - 10 min
3. [Display Configuration](./Display-Configuration.md) - 15 min
4. Try creating your first class!

### Intermediate Path
1. [Property Types](./Property-Types.md) - 20 min read
2. [Static Properties](./Static-Properties.md) - 15 min read
3. [Populate Feature](./Populate-Feature.md) - 20 min read
4. [Quick Reference](./QUICK-REFERENCE-Static-Properties.md) - 5 min
5. Configure advanced properties

### Advanced Path
1. [Architecture](./Architecture.md) - 30 min read
2. [Data Loading](./Data-Loading.md) - 20 min read
3. [Test Fixes](./Test-Fixes-Nov-2025.md) - 30 min read
4. Contribute to the project!

---

## 🗓️ Documentation Versions

### Current Version (November 2025)
- All documentation updated for static properties feature
- 100% English documentation
- Comprehensive test documentation
- Complete migration guide

### Previous Versions
- See [Changelog](../CHANGELOG.md) for version history

---

## ✨ Documentation Quality

### Standards
- ✅ All documents in English
- ✅ Consistent formatting
- ✅ Code examples tested
- ✅ Cross-references verified
- ✅ Up-to-date with code

### Maintenance
- Regular reviews with each release
- User feedback incorporated
- Technical accuracy verified
- Examples kept current

---

**Last Updated**: November 2025  
**Documentation Version**: 1.0.0  
**Project Version**: See [Changelog](../CHANGELOG.md)
